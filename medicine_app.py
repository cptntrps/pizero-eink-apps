...
# Menu options
MENU_OPTIONS = ["Pending Doses", "Skip History", "Adherence", "Exit to Main Menu"]
...
class MedicineApp:
    ...
    def load_medicines(self):
        ...
        try:
            ...
            # Build carousel: MENU first, then all medicines not yet taken today
            self.carousel_items = ["MENU"] + self.medicines
            self.carousel_index = 0
    ...
    def handle_long_press(self):
        ...
        elif self.state in [STATE_PENDING_VIEW, STATE_HISTORY_VIEW, STATE_STATS_VIEW]:
    ...
    def next_pending(self):
        ...
        # TODO: Implement pending navigation
        self.render()
    ...
    def render_menu_options(self):
        ...
        # Current option
        option = MENU_OPTIONS[self.selected_option]
        icons = {"Pending Doses": "📋", "Skip History": "⏭️", "Adherence": "📊", "Exit to Main Menu": "🚪"}
    ...
    def render_pending_view(self):
        ...
        if not self.pending_doses:
            ...
            y = 35
            for dose in self.pending_doses[:3]:  # Show first 3
                draw.text((5, y), f"• {dose['name']}", font=font_body, fill=0)
                y += 15
                window = dose.get('time_window', '').capitalize()
                draw.text((10, y), f"  {window} window", font=font_body, fill=0)
                y += 18

        # Instructions
        draw.text((5, self.epd.width - 12), "Click: Next | Hold 2s: Back", font=font_body, fill=0)
    ...
    def render_history_view(self):
        ...
        font_title = get_font_preset('title')
        font_body = get_font_preset('subtitle')
        ...
        if not self.skip_history:
            ...
            y = 30
            for skip in self.skip_history[:4]:  # Show first 4
                draw.text((5, y), f"{skip['medicine']}", font=font_body, fill=0)
                y += 12
                draw.text((5, y), f"  {skip['date']} - {skip['reason']}", font=font_body, fill=0)
                y += 15
